...
typedef struct {
  ...
  uint8_t tx_data[MAX_DATA_BUFFER_SIZE];
  uint8_t tx_idex;
  bool tx_pkg_cplt_f;
  uint8_t tx_pkg[MAX_PKG_SIZE];
  uint8_t encd_idex;

  uint8_t reg_hdle_stat;
  uint8_t reg_hdle_end;

} Rs485_t;

typedef uint32_t (*rs485_handler_t)(RsFunc_t func, uint16_t addr, uint16_t data, uint8_t len);
...
RsError_t RsDecd(Rs485_t *rs485, RsFunc_t rx_Func, uint8_t *rx_Data, uint8_t rx_Data_len, RsFunc_t *tx_Func, uint8_t *tx_Data,
                           uint8_t *tx_Data_len);
RsError_t RsEncd(Rs485_t *rs485, RsFunc_t Func, uint16_t stat_addr, uint8_t *Data_Num, uint8_t Data_len, uint8_t *tx_Data,
                           uint8_t *tx_Data_len);
